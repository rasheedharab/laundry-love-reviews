...
export default function AdminServices() {
  ...
  const handleSave = async () => {
    const included = form.whats_included.split(",").map((s) => s.trim()).filter(Boolean);
    const payload = {
      name: form.name,
  ...
  const bulkDelete = async () => {
    ...
    setBulkLoading(true);
    const { error } = await supabase.from("services").delete().in("id", Array.from(selected));
    setBulkLoading(false);
    if (error) { toast.error(error.message); return; }
    toast.success(`${selected.size} service${selected.size > 1 ? "s" : ""} deleted`);
    setSelected(new Set());
  ...
  return (
    <div>
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4 pt-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5"><Label>Price (Standard)</Label><Input type="number" value={form.price_standard} onChange={(e) => setForm({ ...form, price_standard: e.target.value })} /></div>
                <div className="space-y-1.5"><Label>Price (Express)</Label><Input type="number" value={form.price_express} onChange={(e) => setForm({ ...form, price_express: e.target.value })} /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5"><Label>Turnaround (Std)</Label><Input value={form.turnaround_standard} onChange={(e) => setForm({ ...form, turnaround_standard: e.target.value })} /></div>
                <div className="space-y-1.5"><Label>Turnaround (Exp)</Label><Input value={form.turnaround_express} onChange={(e) => setForm({ ...form, turnaround_express: e.target.value })} /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5"><Label>Badge</Label><Input value={form.badge} onChange={(e) => setForm({ ...form, badge: e.target.value })} placeholder="e.g. Popular, New" /></div>
      ...
      <div className="rounded-2xl border border-border overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((s) => (
                <tr key={s.id} className={`border-b border-border last:border-0 ${selected.has(s.id) ? "bg-primary/5" : ""}`}>
      ...
      <ConfirmDeleteDialog
        ...
        description="This will permanently delete the selected services."
      />
    </div>
  );
}
